...
export default {
  ...
  created() {
    ipcRenderer.on(types.AVATAR_RESPONSE, (event, base64code) => {
      ...
      this.$message({
        message: this.$t('setting.avatarChanged'),
        type: 'info'
      })
    })
  ...
  methods: {
    ...
    handleUsernameEnter(username) {
      if (username === '') {
        this.$message({
          message: this.$t('setting.usernameNotNone'),
          type: 'alert'
        })
        this.innerUsername = this.username
      } else if (username !== this.username) {
        this.$message({
          message: this.$t('setting.usernameChanged', { username }),
          type: 'info'
        })
    ...
    handleLanguageSelect(index) {
      ...
      this.$message({
        type: 'info',
        message: this.$t('setting.languageChagned')
      })
    },
    ...mapMutations({
